...
async def _get_pokemon_profile(client: AsyncClient, name: str) -> dict[str, Any]:
    """Retrieve information about a single, known Pokémon."""
    name = name.lower()
...
async def _get_pokemon_profiles(
    client: AsyncClient, names: List[str]
) -> dict[str, Any]:
    """Retrieve information about multiple Pokémon.
    Args:
    names (List[str]): The exact names or Pokédex IDs of the Pokémon.
    """
    tasks = {name: _get_pokemon_profile(client, name) for name in names}
